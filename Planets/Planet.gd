...
export (String) var default_colorscheme = ""
...
func _set_colors_from_gradient(mat, grad_var, new_gradient):
	var size = mat.get_shader_param(grad_var).gradient.colors.size()
	var copy = Array(new_gradient).duplicate()
	copy.resize(size)
	mat.get_shader_param(grad_var).gradient.colors = copy
