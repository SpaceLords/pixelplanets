...
func get_seed():
	return $PlanetUnder.material.get_shader_param("seed")
...
func get_material_properties():
	var mu = $PlanetUnder.material
	var ml = $Lakes.material
	var mc = $Clouds.material
	return {
		"pixels": mu.get_shader_param("pixels"),
		"rotation": mu.get_shader_param("rotation"),
		"seed": mu.get_shader_param("seed"),
		"ground_fbm_octaves": mu.get_shader_param("OCTAVES"),
		"ground_fbm_size": mu.get_shader_param("size"),
		"lake_fbm_octaves": ml.get_shader_param("OCTAVES"),
		"lake_fbm_size": ml.get_shader_param("size"),
		"lake_cutoff": ml.get_shader_param("lake_cutoff"),
		"cloud_fbm_octaves": mc.get_shader_param("OCTAVES"),
		"cloud_fbm_size": mc.get_shader_param("size"),
		"cloud_cover": mc.get_shader_param("cloud_cover"),
		"cloud_curve": mc.get_shader_param("cloud_curve"),
		"cloud_stretch": mc.get_shader_param("stretch"),
		"light_border1": mu.get_shader_param("light_border_1"),
		"light_border2": mu.get_shader_param("light_border_2"),
		"light_origin": mu.get_shader_param("light_origin"),
		"dithered": mu.get_shader_param("should_dither"),
	}
